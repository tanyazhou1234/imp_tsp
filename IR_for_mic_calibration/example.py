...
for out_channel in range(1, 2):
    ...
    plt.figure()
    plt.plot(ir[0,:])
    file_path = os.path.join(folder_name, f"imp_SP1_to_MIC{mic_c}.png")
    ...
    plt.figure()
    plt.plot(tsp)
    file_path = os.path.join(folder_name, f"signal_SP1_to_MIC{mic_c}.png")
    plt.savefig(file_path)  # 保存图像
    plt.close()
    ...
    file_path = os.path.join(folder_name, f"signal_spectrum_SP1_to_MIC{mic_c}.png")
    plt.savefig(file_path)  # 保存图像
    plt.close()
    
    # Save data
    fname_imp = os.path.join(folder_name, "imp_fs%d_ch%d.mat" % (int(Fs), int(mic_c)))
